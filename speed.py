...
def main():
    ...
    with sv.VideoSink(args.target_video_path, video_info) as sink:
        while cap.isOpened():
            ...
            # Any preprocessing before model inference can be timed here
            ...
            # Separate classes
            forklift_dets = sv.Detections.from_ultralytics(results)[
                (results.class_id == FORKLIFT_CLASS) &
                (results.confidence > args.confidence_threshold)
            ]
            plate_dets = sv.Detections.from_ultralytics(results)[
                (results.class_id == PLATE_CLASS) &
                (results.confidence > args.confidence_threshold)
            ]
            stop_dets = sv.Detections.from_ultralytics(results)[
                (results.class_id == STOP_SIGN_CLASS) &
                (results.confidence > args.confidence_threshold)
            ]
            ...
            preprocess_ms = (t_pre - t0) * 1000
            inference_ms = (t_inference - t_pre) * 1000
            post_ms = (t_end - t_inference) * 1000

            # New output format (a single print per frame)
            print(f"{frame_index}: {height}x{width} {len(forklift_ids)} forklifts, {len(plate_dets.xyxy)} plate, {(1 if fixed_stop_sign else 0)} stop, {total_ms:.1f}ms Speed: {preprocess_ms:.1f}ms preprocess, {inference_ms:.1f}ms inference, {post_ms:.1f}ms postprocess per image at shape (1, 3, {height}, {width})")
            
            # Removed cv2.imshow; keep waitKey only for quitting
